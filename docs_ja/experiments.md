...
`compare` はアーキテクチャごとに 1 シードの数値を 1 つ報告するだけで、順位を
つけるには足りない。データを固定し、初期化とバッチ順序を 3 つのアーキテクチャで
揃えたまま 10 シードにわたって繰り返すと、最終テスト正解率は次のようになる。

| 構造 | 平均 | 標準偏差 | 最小 | 最大 |
|---|---|---|---|---|
| 浅いモデル | 96.50 | 0.23 | 96.1 | 96.9 |
| 深い MLP | 97.56 | 0.45 | 96.7 | 98.3 |
| CNN | 97.25 | 0.46 | 96.4 | 97.8 |

| 比較 | 成立した回数 | 平均差 |
|---|---|---|
| 深い MLP > 浅いモデル | **10 / 10 シード** | +1.06 pt |
| CNN > 浅いモデル | **9 / 10 シード** | +0.75 pt |
| CNN ≥ 深い MLP | 4 / 10 シード | −0.31 pt |

したがって生き残る主張は「**深さが線形モデルに勝つ**」であり、これは頑健である。
生き残らないのは深い MLP と CNN のあいだの順序づけである。両者の差は各々の
シード間のばらつきより小さく、0.31 ポイントは 360 サンプルのテストセットで
画像およそ 1 枚分にすぎない。

これはこの規模における正直な結果であって、期待外れというわけではない。8×8 の
画像を 3×3 のカーネルが走査しても利用できる並進はほとんどなく、しかも数字は
すでに中央寄せされサイズも正規化されている。畳み込みが勝つ条件がほぼ揃って
いないのである。より大きく前処理の少ない画像では差が開くはずだが、それはまさに
このデータセットでは検証できない主張である。

再現するには `python reference/architecture_trials.py` を実行する。
