...
各デモは自己完結した HTML ページも 1 枚書き出す（`from_scratch.html` と
`compare.html`）。生成しているのは `include/nnscratch/report.hpp` である。
ブラウザで開くだけでよく、インストールするものもサーバを立てる必要もない。

| ファイル | 出力元 | 内容 |
|---|---|---|
| `from_scratch.html` | `from_scratch` | 主要な正解率、損失曲線（対数軸）、train/test 正解率、第 1 層 64 ユニットの重み画像 |
| `compare.html` | `compare` | 3 実験それぞれの損失・正解率チャートと要約表、および学習された畳み込みフィルタ 8 枚 |

**自己完結とは文字どおりの意味である。** CSS も JavaScript もすべてのデータ点も
文書内にインライン展開されており、CDN も `<script src>` も `fetch` も Web フォントも
使わない。オフラインでも、`file://` でも、このリポジトリを見たことがないマシンでも
動き、メールに添付しても壊れない。これはライブラリ本体が従っているのと同じ制約を、
その出力にも適用したものである。

チャートは `<canvas>` に約 100 行の素の JavaScript で描いている。チャート
ライブラリを使わないのは、それが表示時のネットワーク要求か、minify 済みファイルの
リポジトリへの取り込みかのどちらかを意味し、どちらも趣旨に反するからである。

CSV にはない、このページならではの機能:

- チャート上を**ホバー**するとクロスヘアとそのエポックの値が出る
- **凡例をクリック**すると系列を隠せる。色は実行に紐づくので、1 つ隠しても他の色は
  変わらない
- **Show data table** でチャートの背後の数値を表示できる。色が使えない環境でも
  チャートが使えるのはこれによる
- 損失チャートの**対数／線形切り替え**。損失曲線の面白い部分は最初の数エポックで、
  線形軸ではそこが潰れてしまう
- **テーマボタン**と、OS 設定に追従するダークモード

`output/` は gitignore されているので、これらはコミットせず再生成する。
